...
def print_custos(custos):
    r = "custos\n"
    print("t_load", custos["t_load"].shape)
    for idk, k in enumerate(custos["t_load"]):
        for idq, q in enumerate(k):
            print(f"t_load[{idk}][{idq}] = {q}")

    print("t_empty", custos["t_empty"].shape)
    for idk, k in enumerate(custos["t_empty"]):
        for idq, q in enumerate(k):
            print(f"t_empty[{idk}][{idq}] = {q}")

    print("E_load", custos["E_load"].shape)

    for idk, k in enumerate(custos["E_load"]):
        for idq, q in enumerate(k):
            for ida, a in enumerate(q):
                print(f"E_load[{idk}][{idq}][{ida}] = {a}")
    ...
    # for idk, k in enumerate(custos["E_empty"]):
    #     for idq, q in enumerate(k):
    #         print(f"E_empty[{idk}][{idq}] = {q}")
    ...
    for y in range(num_fileiras):
        for x in range(num_posicoes_nivel_inferior):
            ...
            if x != num_posicoes_nivel_inferior - 1:
                posicoes.append(
                    (y + 1, 2, x + 1)
                )  # nível superior, entre duas inferiores
...
def gerar_modelo(
    ...
    W = model.addVars(
        len(S), len(Phi), len(Phi), len(A), vtype=GRB.BINARY, name="W"
    )
    ...
    for a in range_A_in:
        model.addConstr(
            quicksum(W[s, 0, q, a] for s in S for q in range_Phi if q not in I)
            == 1,
    ...
    for a in range_A:
        if a not in range_A_out:
            model.addConstr(
                quicksum(W[s, k, len(Phi) - 1, a] for s in S for k in range_Phi)
                == 0,
        ...
        for s in S:
            model.addConstr(
                ...
                - (
                    tau[s]
                    + quicksum(
                        W[s, k, len(Phi) - 1, a] * t_load[k, len(Phi) - 1]
                        for k in range_Phi
                    )
                )
                <= (1 - x[s, len(Phi) - 1, a]) * M,
    ...
    for ids, s in enumerate(S):
        if ids != 0:
            model.addConstr(
                ...
                >= S[ids - 1]
                + quicksum(
                    t_empty[k, q] * V[ids - 1, k, q]
                    for k in range_Phi
                    for q in range_Phi
                )
                + quicksum(
                    t_load[k, q] * W[ids - 1, k, q, a]
                    for k in range_Phi
                    for q in range_Phi
                    for a in range_A
                ),
    ...
    for k in range_Phi:
        for s in S:
            model.addConstr(
                quicksum(x[s, k, a] for a in range_A) <= 1,
    ...
    for s in S:
        model.addConstr(
            quicksum(
                W[s, len(Phi) - 1, q, a] for q in range_Phi for a in range_A
            )
            == 0,
    ...
    for k in range_Phi:
        for ids, s in enumerate(S):
            if ids != 0:
                model.addConstr(
                    quicksum(W[s, k, q, a] for q in range_Phi)
                    == quicksum(V[ids - 1, q, k] for q in range_Phi),
    ...
    for k in range_Phi:
        for ids, s in enumerate(S):
            if ids != 0:
                model.addConstr(
                    quicksum(V[s, k, q] for q in range_Phi)
                    - quicksum(
                        W[ids - 1, q, k, a] for q in range_Phi for a in range_A
                    )
                    <= 0,
        ...
        for idk, q in enumerate(Psi2):
            if idk != 0:
                model.addConstr(
                    2
                    * quicksum(
                        W[s, k, q, a] for k in range_Phi for a in range_A
                    )
                    <= quicksum(
                        x[s, idk - 1, a] + x[s, idk + 1, a] for a in range_A
                    ),
...
def test():
    for teste in testes:
        ...
        custos = gerar_custos_de_movimentacao(
            posicoes["Phi"], teste["num_bobinas"]
        )
